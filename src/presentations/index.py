...
class OpenSearchIndex:
    ...
    def create_index(self, index_name: str, mapping: Dict[str, Any], overwrite: bool = False) -> None:
        """
        ...
        if exists:
            if overwrite:
                self.logger.warning(f"Index {index_name} already exists. Overwriting...")
                self.client.indices.delete(index=index_name)
            else:
                self.logger.info(f"Index {index_name} already exists. Use overwrite=True to overwrite.")
    ...
    def _validate_index_name(self, index_name: str) -> None:
        ...
        if not re.match(r'^[a-z0-9_+-]+$', index_name):
            raise ValueError("Index name must contain only lowercase letters, numbers, underscores, and hyphens")
    ...
    def bulk_index_documents(self, index_name, documents, id_field, batch_size: int = 500, max_retries: int = 3, show_progress: bool = True) -> Dict[str, Any]:
        """
        ...
        iterator = tqdm(batches, desc="Bulk indexing", disable=not show_progress) if show_progress else batches
        ...
        self.logger.info(f"Bulk indexing completed. Success: {success_count}, Failed: {failed_count}, Total: {total_docs}")
        return result

    def _execute_bulk_index(self, actions, operation_type, max_retries: int = 3, expected_count: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        while retry_count < max_retries:
            try:
                ...
                if response.get("errors"):
                    results = [item.get(operation_type, {}) for item in response["items"] if operation_type in item]
                    failed_items = [{"id": r.get("_id"), "error": r.get("error")} for r in results if r.get("status", 0) >= 400]
                ...
                if retry_count < max_retries:
                    self.logger.warning(f"Bulk operation failed (attempt {retry_count}/{max_retries}): {e}")
                    time.sleep(2 ** retry_count)
                    continue
                
                self.logger.error(f"Bulk operation failed after {max_retries} retries: {e}")
                bulk_result = {
                    "success": 0,
    ...
    def search_similar_vectors(self, index_name: str, query_vector: List[float], k: int = 10) -> List[Dict[str, Any]]:
        """
